...
var label = document.querySelectorAll('.tabs label');
var content = document.querySelectorAll('.tabs section p img');

for(var i in categories){
	content[+i].innerText = 'Новый товар';
	label[+i].innerText = categories[+i].categoryName;
	for(var j in products)
	if(categories[+i].categoryId == products[+j].categoryId)
			content[+i].innerText += ' ' + products[+j].productName;
}